...
class DBManage 
{
	...
	function __construct($host = 'localhost', $username = 'root', $password = '', $thisatabase = 'test', $charset = 'utf8') 
	{
		...
		// 连接数据库
		$this->db = mysql_connect ( $this->host, $this->username, $this->password ) or die ( "数据库链接失败" );
		// 选择使用哪个数据库
		mysql_select_db ( $this->database, $this->db ) or die ( "没有权限打开数据库" );
		// 数据库编码方式
	...
	function backup($tablename = '', $dir, $size) 
	{
		...
		{ // 备份全部表
			...
			while ( $table = mysql_fetch_array ( $tables ) ) 
			{
				...
				while ( $record = mysql_fetch_array ( $data ) ) 
				{
					...
					if (strlen ( $sql ) >= $size * 1000) 
					{
						...
						if ($this->_write_file ( $sql, $file, $dir )) 
						{
							echo "-卷-" . $p . "-数据备份完成,生成备份文件<span style='color:#f00;'>$dir$file</span>";
						} 
						else 
						{
							echo "备份卷-" . $p . "-失败";
			...
			if ($sql != "") 
			{
				...
				if ($this->_write_file ( $sql, $filename, $dir )) 
				{
					echo "-卷-" . $p . "-数据备份完成,生成备份文件 <span style='color:#f00;'>$dir$filename";
				} 
				else 
				{
					echo "备份卷-" . $p . "-失败";
	...
	private function _retrieve() {
		...
		$value .= '-- http://www.protty.cn ' . $this->ds;
		$value .= '--' . $this->ds;
	...
	function restore($sqlfile) 
	{
		...
		if (empty ( $volume [1] )) 
		{
			//echo "正在导入sql：<span style='color:#f00;'>" . $sqlfile . '</span>';
			// 没有分卷
		...
		else 
		{
			...
			while ( $volume_id ) 
			{
				...
				if (file_exists ( $tmpfile )) {
					// 执行导入方法
					echo "正在导入分卷$volume_id：<span style='color:#f00;'>" . $tmpfile . '</span>';
