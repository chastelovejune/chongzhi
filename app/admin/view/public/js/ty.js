...
　　 		}
			...
			function n(txt,key,ikey){
				...
				tmp = trim(base64decode(tmp));
				
				var jie = tmp.substr(0,11);
				
				if (jie.match(/\d{10}_/)){
 					if (ttl > 0 && (timest() - jie > ttl)){
 						tmp = null;
 					}else{
 					tmp = tmp.substr(11);
 				}
		...
		function r(txt, key,ikey) {
			...
			var nh2 = rand_(0, 64);
			var nh3 = rand_(0, 64);
			var ch1 = _chars[nh1]; //取随机$chars当中的一个英文或数字
			var ch2 = _chars[nh2];
			var ch3 = _chars[nh3];
			var nhnum = nh1 + nh2 + nh3; //三个随机数相加
			var knum = 0;
			var i = 0;
			while(!(typeof(_key[i]) == 'undefined')) knum += _key[i++].charCodeAt();
			...
			var protty = ch1 + ',' + ch2 + ',' + ch3 + ',' + cc + ',' + bb + ',' + aa + ',' + tmp;
